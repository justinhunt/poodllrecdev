 require(['jquery','core/log','filter_poodll/poodll_mediarecorder'], function($,log, pmr) {
	...
	params['fallback']='iosupload'; //'warning' or 'upload' or 'iosupload'
	...
	if(('appid' in urlparams)){params['appid']=urlparams['appid'];}
	...
	if(('fallback' in urlparams)){params['fallback']=urlparams['fallback'];}
	...
	if(('transcoder' in urlparams)){params['transcoder']=urlparams['transcoder'];}
	if(('transcribe' in urlparams)){params['transcribe']=parseInt(urlparams['transcribe']);}
	if(('subtitle' in urlparams)){params['subtitle']=parseInt(urlparams['subtitle']);}
	...
	//what to do if the browser is not supportive ...(and user has not specifically requested upload)
     //lets enable Desktop Safari
     params['html5ondsafari']=true;
     if(!pmr.supports_current_browser(params) && params['media_skin']!='upload'){
        switch(params['fallback']){
            case 'warning':
                params['media_skin']='warning';
                break;
            case 'upload':
                params['media_skin']='upload';
                break;
            case 'iosupload':
            default:
                if(pmr.is_ios()){
